...
//	handlers
	...
	{
		$rs = DB::Query("SELECT COUNT(dbrf.aldevindiv.aldevindiv_id) AS totelementos
FROM dbrf.aldevindiv
WHERE dbrf.aldevindiv.di_funcionario <> '800';");
...
$rs = DB::Query("SELECT
	count(dbrf.almovinddia.almovinddia_id) as totlegalizados
FROM
	dbrf.almovinddia
WHERE
	dbrf.almovinddia.md_legalizado = 1 AND dbrf.almovinddia.sys_status = 0;");
...
echo "<h5><strong style='color: rgb(255, 255, 255);'><em>Elementos en bodega</em></strong></h5>";;
	...
	{
		$rs = DB::Query("SELECT
    CONCAT_WS(' | ', COUNT(dbrf.aldevindiv_sol.transact_id), dbrf.aldevindiv_sol_type.sol_lnk) as numsol
FROM
    dbrf.aldevindiv_sol
INNER JOIN
    dbrf.aldevindiv_sol_type
ON 
    dbrf.aldevindiv_sol.sol_type = dbrf.aldevindiv_sol_type.sol_id
GROUP BY
    dbrf.aldevindiv_sol.sol_type");
